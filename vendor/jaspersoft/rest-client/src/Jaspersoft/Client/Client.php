...
class Client
{
    ...
    /**
     * View jasperServer dashboard on any web page using iframe or directly redirect on other page
     * Author :Sujeet Kumar yadav
     * Date : 16/07/2015
     * Param: Full path of your Dashboard 
     */

    public function runDashboard($path=null)
    {//$filePath = preg_replace("(/)", "%2F", $path);
    $fullPath=$this->serverUrl."/dashboard/viewer.html?decorate=no&viewAsDashboardFrame=true&j_username=".$this->username;
    $fullPath.="&j_password=".$this->password."#";
    $fullPath.=$path ;
    return $fullPath;
    }
}